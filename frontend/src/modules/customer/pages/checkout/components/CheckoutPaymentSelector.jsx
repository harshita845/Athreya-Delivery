...
const CheckoutPaymentSelector = React.memo(function CheckoutPaymentSelector({
  ...
  return (
    <>
      {/* Wallet Section */}
      {walletBalance > 0 && (
        <motion.div className="bg-white rounded-2xl p-4 border border-[#1a6e2e]/20 overflow-hidden relative">
          <div className="flex items-center justify-between mb-2">
            <div className="flex items-center gap-2">
              <div className="h-8 w-8 rounded-full bg-[#1a6e2e]/10 flex items-center justify-center">
                <Wallet size={16} className="text-[#1a6e2e]" />
              </div>
              <div>
            ...
            <button
              ...
              className={`w-12 h-6 rounded-full transition-all duration-300 relative flex items-center px-1 ${
                useWallet ? "bg-[#1a6e2e]" : "bg-slate-200"
              }`}>
              <motion.div
                animate={{ x: useWallet ? 24 : 0 }}
                className="h-4 w-4 rounded-full bg-white border border-[#1a6e2e]/20"
              />
            </button>
          </div>
          {useWallet && (
            <motion.div
              ...
              <div className="flex justify-between items-center bg-[#1a6e2e]/10 p-2 rounded-xl">
                <span className="text-[11px] font-bold text-slate-600 uppercase">
                  Amount to be used
                </span>
                <span className="text-[13px] font-black text-[#1a6e2e]">
                  ₹{walletAmountToUse}
                </span>
              </div>
            </motion.div>
              ...
              <button
                ...
                className={`w-full p-3 rounded-xl border-2 transition-all flex items-center gap-3 ${
                  selectedPayment === method.id
                    ...
                    : "border-[#1a6e2e]/20 bg-white hover:border-[#1a6e2e]"
                }`}>
                <div
                  className={`h-10 w-10 rounded-full flex items-center justify-center ${
                    selectedPayment === method.id ? "bg-[#1a6e2e]/20" : "bg-[#1a6e2e]/5"
                  }`}>
                  <Icon
                    size={18}
                    className={
                      selectedPayment === method.id ? "text-[#1a6e2e]" : "text-slate-600"
                    }
                  />
                </div>
                <div className="flex-1 text-left">
                  <p
                    className={`font-bold text-sm ${
                      selectedPayment === method.id ? "text-[#1a6e2e]" : "text-slate-800"
                    }`}>
                    {method.label}
                  </p>
                ...
                <div
                  className={`h-5 w-5 rounded-full border-2 flex items-center justify-center ${
                    selectedPayment === method.id ? "border-[#1a6e2e]" : "border-slate-300"
                  }`}>
                  {selectedPayment === method.id && (
                    <div className="h-3 w-3 rounded-full bg-[#1a6e2e]" />
                  )}
                </div>
              </button>
            );
