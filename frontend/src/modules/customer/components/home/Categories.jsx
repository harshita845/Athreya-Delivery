...
const categories = [
    ...
    { id: 2, name: 'Vegetables', image: 'https://images.unsplash.com/photo-1566385101042-1a0aa0c1268c?q=80&w=200&auto=format&fit=crop', color: 'bg-[#1a6e2e]/10' },
    { id: 3, name: 'Dairy', image: 'https://images.unsplash.com/photo-1628088062854-d1870b4553da?q=80&w=200&auto=format&fit=crop', color: 'bg-[#1a6e2e]/10' },
    ...
    { id: 9, name: 'Baby Care', image: 'https://images.unsplash.com/photo-1519689680058-324335c77eba?q=80&w=200&auto=format&fit=crop', color: 'bg-[#1a6e2e]/10' },
        ...
        <section className="py-8 bg-white">
            <div className="container w-full max-w-[1920px] mx-auto px-4 md:px-[50px]">
                <div className="flex items-center justify-between mb-8">
                    <h2 className="text-2xl md:text-3xl font-bold tracking-tight text-[#1a6e2e]">Shop by Category</h2>
                    <a href="/categories" className="text-sm font-bold text-[#1a6e2e] hover:text-[#1a6e2e] flex items-center gap-1 transition-colors">
                ...
                <div className="flex gap-6 overflow-x-auto pb-4 scrollbar-hide snap-x -mx-4 px-4 md:mx-0 md:px-0 md:flex md:flex-wrap md:gap-8 md:justify-start">
                    {categories.slice(0, 8).map((category) => (
                        ...
                        >
                            <div className={`h-36 w-36 rounded-full ${category.color} p-4 flex items-center justify-center border border-[#1a6e2e]/20 transition-all duration-300 group-hover:scale-110 group-hover:border border-[#1a6e2e]/20 border border-slate-100`}>
                                <img
                                    src={category.image}
                                    alt={category.name}
                                    className="w-full h-full object-contain drop-border border-[#1a6e2e]/20 mix-blend-multiply"
                                />
                            </div>
                            <span className="text-lg font-medium text-slate-700 group-hover:text-[#1a6e2e] transition-colors">
                                {category.name}
                            </span>
                        </a>
                    ))}
