    ...
    <div className="w-full mb-5 -mt-[24px] md:mt-3 overflow-hidden relative group z-20">
      <div
        ...
        <div className="absolute left-4 lg:left-10 top-[58%] -translate-y-1/2 z-20 hidden md:flex">
          <button
            onClick={() => scroll("left")}
            className="h-10 w-10 bg-white/90  border border-[#1a6e2e]/20 rounded-full flex items-center justify-center border border-gray-100 cursor-pointer hover:bg-white text-[#1a6e2e] transition-all active:scale-90">
          ...
          {categories.map((cat, idx) => {
            ...
            return (
              <div
                ...
                <div
                  className="relative w-[74px] h-[84px] md:w-[104px] md:h-[116px] lg:w-[120px] lg:h-[132px] rounded-[18px] md:rounded-[22px]  border flex items-start justify-center p-1.5 md:p-2 transition-all duration-300 group-hover/item:-translate-y-1 group-hover/item: overflow-hidden smooth-transform"
                  style={{
                  ...
                  <img
                    ...
                    alt={cat.name}
                    loading="lazy"
                    className="absolute left-1/2 top-2.5 md:top-3 z-10 h-[56px] w-[56px] md:h-[64px] md:w-[64px] -translate-x-1/2 object-contain drop- mix-blend-multiply group-hover/item:scale-110 transition-transform duration-500"
                  />
                  <div className="absolute inset-x-1.5 md:inset-x-2 bottom-1.5 z-20 text-center">
                    <span className="block text-[9px] md:text-[10px] lg:text-[11px] font-semibold text-[#1f2b20] leading-tight whitespace-nowrap overflow-hidden text-ellipsis drop- group-hover/item:text-[#1a6e2e] transition-colors">
                      {cat.name}
                    </span>
                  </div>
                </div>
